...
from loader.universal_loader import universal_loader_v2
...
def initialize():
    ...
    documents = universal_loader_v2(DATA_PATH)
    chunks = split_documents(documents)
    add_to_chroma(chunks, db)


def update():
    llm_type = os.getenv('LLM_MODEL_TYPE')
...
def group_documents_by_title(titles, contents):
    # title에서 파일 경로만 추출하여 그룹화
    grouped_data = defaultdict(list)

    for title, content in zip(titles, contents):
        base_title = title.rsplit(":")[0]
        grouped_data[base_title].append(content)

    # 결과적으로 그룹화된 title과 합쳐진 content 반환
    grouped_documents = [
        {"title": title, "content": "<br /><br />".join(grouped_data[title])}
        for title in grouped_data
    ]

    return grouped_documents


def get_documents(embedding_model):
    if embedding_model.lower() in ['gpt']:
        embeddings = Embeddings(model_name="openai", api_key=OPENAI_API_KEY)
        db_path = VECTOR_DB_OPENAI_PATH
    elif embedding_model.lower() in ['ollama', 'llama']:
        embeddings = Embeddings(model_name="ollama", api_key=OPENAI_API_KEY)
        db_path = VECTOR_DB_OLLAMA_PATH
    else:
        raise ValueError("Unsupported embedding model specified.")
    ...
    data = [
        {"id": doc.get("id", idx + 1), "title": doc.get("title"), "content": content}
        for idx, (doc, content) in enumerate(zip(documents.get('metadatas', []), contents))
    ]

    titles = []
    for item in data:
        title = item['title'] if item['title'] else str(item['id'])
        titles.append(title)

    grouped_documents = group_documents_by_title(titles, contents)

    return grouped_documents


def add_documents(sources: list[str]):
    llm_type = os.getenv('LLM_MODEL_TYPE')

    print("Model Type : ", llm_type)

    embeddings = Embeddings(model_name=llm_type, api_key=OPENAI_API_KEY)
    embedding_function = embeddings.get_embedding_function()
    ...
    # create (or update) the data store
    documents = universal_loader_v2(*sources)
    chunks = split_documents(documents)
    add_to_chroma(chunks, db)
